...
public class controladorPrincipal extends HttpServlet {
    ...
    //-------------------------------------------------------------------------------------------------------------------------------------------//
                ...
                if (request.getParameter("boton").compareTo("GRABAR") == 0) {
                    String operacion = request.getParameter("operacion");
                    String strDate1 = request.getParameter("xfeci");
                    String strDate2 = request.getParameter("xfecf");
                    Date xfeci = Date.valueOf(strDate1);
                    Date xfecf = Date.valueOf(strDate2);
                    Cursos curso = new Cursos();
                    ...
                    curso.setFec_ini(xfeci);
                    curso.setFec_fin(xfecf);
     ...
     //-------------------------------------------------------------------------------------------------------------------------------------------//
                ...
                IMatriculaDAO dao = new MatriculaDAOImpl();
                arrCursos = dao.buscarCursos();
                arrMatriculas = dao.buscarMatriculas();
                request.setAttribute("arrCursos", arrCursos);
            ...
            } else if (accion.equals("buscarPorNroDoc")) {
                String nro_doc = request.getParameter("nro_doc");
                List<Matriculas> arrMatriculas = new ArrayList<Matriculas>();
                IMatriculaDAO dao = new MatriculaDAOImpl();
                arrMatriculas = dao.buscarPorNroDoc(nro_doc);
                request.setAttribute("arrMatriculas", arrMatriculas);
                request.getRequestDispatcher("/mantenimientos/listadoMatriculas.jsp").forward(request, response);
            } else if (accion.equals("NuevoEliminarMatricula")) {
                if (request.getParameter("boton").compareTo("Nueva Matricula") == 0) {
                    Matriculas matricula = new Matriculas();
                    request.setAttribute("matricula", matricula);
                    request.setAttribute("operacion", "INSERT");
                    request.setAttribute("titulo", "Nueva Matricula");
                    request.getRequestDispatcher("/mantenimientos/editarMatriculas.jsp").forward(request, response);
                } else {
                    String[] datos = request.getParameterValues("xcod");
                    IMatriculaDAO dao = new MatriculaDAOImpl();
                    dao.eliminar(datos);
                    request.getRequestDispatcher("/controladorPrincipal?accion=listadoMatriculas").forward(request, response);
                }
            } else if (accion.compareTo("GRABAR_MATRICULA") == 0) {
                if (request.getParameter("boton").compareTo("GRABAR") == 0) {
                    String operacion = request.getParameter("operacion");
                    Date fecha = Date.valueOf(request.getParameter("xfecm"));
                    Matriculas matricula = new Matriculas();
                    matricula.setCodigo(Integer.parseInt(request.getParameter("xcod")));
                    matricula.setNro_doc(request.getParameter("xnrodoc"));
                    matricula.setFecha(fecha);
                    matricula.setCodigo_alumno(Integer.parseInt(request.getParameter("xcodal")));
                    matricula.setTotal(Double.parseDouble(request.getParameter("xtotal")));
                    matricula.setEstado(request.getParameter("xest"));
                    if (operacion.equals("INSERT")) {
                        IMatriculaDAO dao = new MatriculaDAOImpl();
                ...
                request.getRequestDispatcher("/controladorPrincipal?accion=listadoMatriculas").forward(request, response);
            } else if (accion.compareTo("modificarMatricula") == 0) {
                String xcod = request.getParameter("xcod").trim();
                IMatriculaDAO dao = new MatriculaDAOImpl();
                Matriculas matricula = dao.buscar(Integer.parseInt(xcod));
                request.setAttribute("matricula", matricula);
                request.setAttribute("operacion", "UPDATE");
                request.setAttribute("titulo", "Modificar matricula");
                request.getRequestDispatcher("/mantenimientos/editarMatriculas.jsp").forward(request, response);
                ...
                if (request.getParameter("boton").compareTo("GRABAR") == 0) {
                    ...
                    if (rpta) {
                        response.sendRedirect("/WebSistema/controladorPrincipal?accion=listadoMatriculas");
                    } else {
